...
function getBlockElement(
  ...
  editor.getEditorState().read(() => {
    ...
    while (index >= 0 && index < topLevelNodeKeys.length) {
      ...
      const actualTop = point - marginTop - anchorElementRect.top - SPACE;
      const actualBottom = point + elem.offsetHeight + marginBottom;

      if (event.y >= actualTop && event.y <= actualBottom) {
        blockElem = elem;
        prevIndex = index;
        break;
      }
...
function useDraggableBlockMenu(
  ...
  return createPortal(
    <>
      <div
        className={`absolute top-0 left-0 will-change-transform cursor-grab active:cursor-grabbing opacity-0 ${DRAGGABLE_BLOCK_MENU_CLASSNAME}`}
        ref={menuRef}
        draggable={true}
      ...
      <div
        className="pointer-events-none bg-blue-500 h-1 absolute left-0 top-0 opacity-0 will-change-transform"
        ref={targetLineRef}
      />
    </>,
    anchorElem,
